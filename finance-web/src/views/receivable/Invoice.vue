...
const loadData = async () => {
  loading.value = true
  try {
    const res = await request({ url: '/receivable/invoice/page', method: 'get', params: { ...query, pageNum: pager.pageNum, pageSize: pager.pageSize } })
    data.list = res.data.records
    data.total = res.data.total || 0
  } finally { loading.value = false }
}
const onAdd = () => { Object.assign(form, { id: null, invoiceNo: 'INV-' + Date.now(), invoiceDate: new Date().toISOString().substr(0, 10) }); dialog.visible = true; dialog.title = '新增' }
